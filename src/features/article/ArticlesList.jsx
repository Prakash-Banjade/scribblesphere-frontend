...
const ArticlesList = () => {
  ...
  const articles = Array.isArray(data)? data?.map((data) => {
    return <SingleArticle article={data} isAuthor key={data._id} showContent />;
  }) : <p className="fw-500 color-ccc font-blog" style={{marginLeft: '20px', fontSize: '1.4rem'}}>No articles has been published yet. Be the fist to post.</p>
  return (
